...
func initiatorEncHandshake(conn io.ReadWriter, prv *ecdsa.PrivateKey, remote *ecdsa.PublicKey, netid uint64, version uint) (s secrets, err error) {
	...
	authPacket, err := writeHandshakeMsg(authMsg, h, conn)
	if err != nil {
		return s, err
	}
...
func writeHandshakeMsg(msg interface{}, h *encHandshake, w io.Writer) ([]byte, error) {
