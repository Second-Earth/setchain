...
define build
	@go build -ldflags " \
	-X github.com/Second-Earth/setchain/cmd/utils.commit=$(shell cat commit_hash.txt) \
	-X github.com/Second-Earth/setchain/cmd/utils.date=$(shell date '+%Y-%m-%d-%H:%M:%S') \
	-X 'github.com/Second-Earth/setchain/cmd/utils.goversion=$(shell go version)'" \
...
# build all targets 
.PHONY: all
#all:check  build_set build_setfinder
all:build_set build_setfinder
...
# build setfinder
.PHONY: build_setfinder
build_setfinder:
	#commit_hash check
	@echo "Building setfinder."
	$(call build,setfinder)
