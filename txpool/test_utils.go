...
func generateAccount(t *testing.T, name common.Name, managers ...*am.AccountManager) *ecdsa.PrivateKey {
	...
	for _, m := range managers {
		if err := m.CreateAccount(common.Name("set.founder"), name, common.Name(""), 0, 0, common.BytesToPubKey(pubkeyBytes), ""); err != nil {
			t.Fatal(err)
		}
	}
	return key
...
func setupTxPool(assetOwner common.Name) (*TxPool, *am.AccountManager) {
	...
	asset.IssueAsset("set", 0, 0, "zz", new(big.Int).SetUint64(params.Set), 10, assetOwner, assetOwner, big.NewInt(1000000), common.Name(""), "")
