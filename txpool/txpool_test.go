...
// 	// issue asset
// 	if _, err := asset.IssueAsset("set", 0, 0, "zz", new(big.Int).SetUint64(params.set), 10, common.Name(""), fname, new(big.Int).SetUint64(params.set), common.Name(""), ""); err != nil {
// 		t.Fatal(err)
// 	}

// 	// add balance
// 	if err := manager.AddAccountBalanceByName(fname, "set", new(big.Int).SetUint64(params.set)); err != nil {
// 		t.Fatal(err)
// 	}

// 	if err := manager.AddAccountBalanceByName(tname, "set", new(big.Int).SetUint64(params.set)); err != nil {
// 		t.Fatal(err)
// 	}
...
func TestTransactionChainFork(t *testing.T) {
	...
	resetAsset := func() {
		...
		if err := newmanager.CreateAccount(common.Name("set"), fname, common.Name(""), 0, 0, common.BytesToPubKey(crypto.FromECDSAPub(&fkey.PublicKey)), ""); err != nil {
			t.Fatal(err)
		}
		if err := newmanager.CreateAccount(common.Name("set"), tname, common.Name(""), 0, 0, common.BytesToPubKey(crypto.FromECDSAPub(&tkey.PublicKey)), ""); err != nil {
			t.Fatal(err)
		}
		asset := asset.NewAsset(statedb)

		asset.IssueAsset("set", 0, 0, "zz", new(big.Int).SetUint64(params.set), 10, fname, fname, big.NewInt(1000000), common.Name(""), "")
