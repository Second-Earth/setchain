...
import (
	...
	"github.com/Second-Earth/setchain/processor/vm/runtime"
...
func createAccount(account *accountmanager.AccountManager, name string) error {
	if err := account.CreateAccount(common.Name("set"), common.Name(name), "", 0, 2, common.HexToPubKey("12345"), ""); err != nil {
...
func issueAssetAction(ownerName, toName common.Name) *types.Action {
	...
	action := types.NewAction(types.IssueAsset, ownerName, common.Name("set.asset"), 0, 0, 0, big.NewInt(0), b, nil)
	return action
}
...
func TestAsset(t *testing.T) {
	...
	if err := createAccount(account, "set.asset"); err != nil {
		return
	}
