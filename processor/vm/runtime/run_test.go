...
func TestRunCode(t *testing.T) {
	...
	//
	toName := common.Name("set.asset")

	//fmt.Println("in TestRunCode3 ...")
	if err := account.CreateAccount(common.Name("set"), senderName, "", 0, 0, senderPubkey, ""); err != nil {
		fmt.Println("create sender account error\n", err)
		return
	}

	if err := account.CreateAccount(common.Name("set"), receiverName, "", 0, 0, receiverPubkey, ""); err != nil {
		fmt.Println("create receiver account error\n", err)
		return
	}

	if err := account.CreateAccount(common.Name("set"), toName, "", 0, 0, receiverPubkey, ""); err != nil {
