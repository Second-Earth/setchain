	...
	check := func(shouldPanic bool, arg interface{}) {
		defer func() {
			...
			if shouldPanic && err == nil {
				t.Errorf("Subscribe should've panicked for %#v", arg)
			}
			if !shouldPanic && err != nil {
				t.Errorf("Subscribe shouldn't have panicked for %#v", arg)
		...
		client.FtSubscribe(context.Background(), arg, "foo_bar")
	}
	check(true, nil)
	check(true, 1)
...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("set", new(NotificationTestService))
	defer server.Stop()
	client := dialInProc(server)
	defer client.Close()
	...
	nc := make(chan int)
	count := 10
	sub, err := client.FtSubscribe(context.Background(), nc, "someSubscription", count, 0)
	if err != nil {
...
// In this test, the connection drops while FtSubscribe is
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("set", service)
	defer server.Stop()
	client := dialInProc(server)
	defer client.Close()
	...
	go func() {
		sub, err = client.FtSubscribe(context.Background(), nc, "hangSubscription", 999)
		errc <- err
	}()
	...
	case err := <-errc:
		if err == nil {
			t.Errorf("FtSubscribe returned nil error after Close")
		}
		if sub != nil {
			t.Error("FtSubscribe returned non-nil subscription after Close")
		}
	case <-time.After(1 * time.Second):
		t.Fatalf("FtSubscribe did not return within 1s after Close")
	}
}
...
func TestClientNotificationStorm(t *testing.T) {
	...
	doTest := func(count int, wantError bool) {
		...
		// very quickly.
		nc := make(chan int)
		sub, err := client.FtSubscribe(ctx, nc, "someSubscription", count, 0)
		if err != nil {
			t.Fatal("can't subscribe:", err)
		}
		...
		for i := 0; i < count; i++ {
			...
			var r int
			err := client.CallContext(ctx, &r, "ft_echo", i)
			if err != nil {
				if !wantError {
...
func (l *flakeyListener) Accept() (net.Conn, error) {
	...
	if err == nil {
		...
		time.AfterFunc(timeout, func() {
			fmt.Println(fmt.Sprintf("killing conn %v after %v", c.LocalAddr(), timeout))
			c.Close()
		})
	}
	return c, err
