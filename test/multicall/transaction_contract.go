...
import (
	...
	testcommon "github.com/Second-Earth/setchain/test/common"
	"github.com/Second-Earth/setchain/types"
	"github.com/Second-Earth/setchain/utils/abi"
...
var (
	...
	founderAccount = common.Name("setchain.founder")
	accAccount     = common.Name("setchain.account")
	feeAccount     = common.Name("setchain.fee")
...
func transferMultiAssetByContractFromA2B() {
	...
	assets := make([]*processor.SingleAsset, 0)
	assets = append(assets, &processor.SingleAsset{1, big.NewInt(1)})
	data := &processor.MultiAssetCallAction{
		assets,
		input,
	}
	...
	key_0 := types.MakeKeyPair(a_author_0_priv, []uint64{0})
	aNonce++
	sendTransferTx(types.MultiAssetCall, normal_a, contract_a, aNonce, assetID, big.NewInt(0), payload, []*types.KeyPair{key_0})
}
