...
func GasPrice() (*big.Int, error) {
	gp := big.NewInt(0)
	err := ClientCall("set_gasPrice", gp)
	return gp, err
}
...
func SendRawTx(rawTx []byte) (common.Hash, error) {
	...
	err := ClientCall("set_sendRawTransaction", hash, hexutil.Bytes(rawTx))
	return *hash, err
}
...
func GetBlockAndResult(blockNr rpc.BlockNumber) (*types.BlockAndResult, error) {
	...
	err := ClientCall("set_getBlockAndResultByNumber", result, blockNr)
	return result, err
}
...
func defaultDataDir() string {
	...
	if home != "" {
		if runtime.GOOS == "darwin" {
			return filepath.Join(home, "Library", "set_ledger")
		} else if runtime.GOOS == "windows" {
			return filepath.Join(home, "AppData", "Roaming", "set_ledger")
		} else {
			return filepath.Join(home, ".set_ledger")
		}
	}
