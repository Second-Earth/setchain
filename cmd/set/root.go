...
import (
	...
	"github.com/Second-Earth/setchain/metrics/influxdb"
...
var RootCmd = &cobra.Command{
	Use:   "set",
	Short: "set is a Leading High-performance Ledger",
	Long:  `set is a Leading High-performance Ledger`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		log.Info("set node", "version", utils.FullVersion())

		node, err := makeNode()
		if err != nil {
			log.Error("set make node failed.", "err", err)
			return
		}

		if err := registerService(node); err != nil {
			log.Error("set register service failed.", "err", err)
			return
		}

		if err := startNode(node); err != nil {
			log.Error("set start node failed.", "err", err)
			return
		}
...
func registerService(stack *node.Node) error {
	return stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return setservice.New(ctx, ftCfgInstance.FtServiceCfg)
	})
}
